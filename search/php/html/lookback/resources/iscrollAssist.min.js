(function($,window){
	...
	var exportsMethods = {
		...
		newVerScrollForPull : function(wrapper,pulldownAction,pullupAction,editer,opts,pullText){
			...
			var $pulldown = $scroller.find('#pullDown'),
				$pullup   = $scroller.find('#pullUp'),
				pullupOffset   = 0,
				pulldownOffset = 0;
			...
			scrollObj.on('scrollMove',function(){
				...
				if ($pulldown.length>0 && this.y > 5 && !$pulldown.hasClass('flip')) {
					...
					$pullup.find('#pullUpLabel').html(releaseToLoading);
			...
			scrollObj.on('scrollEnd',function(){
				...
				if ($pulldown.length>0 && $pulldown.hasClass('flip')) {
					...
					$pullup.find('#pullUpLabel').html(loading);
