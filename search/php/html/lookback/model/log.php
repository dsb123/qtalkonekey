...
/*    $log_path = dirname(__FILE__)."/log";
    if(!file_exists($log_path)){
        if(!mkdir($log_path,777)){
           send_mail("不能创建目录",$log_path);
        }
    }
    if(!is_writable($log_path)){
        chmod($log_path,777);
    }
    $log_file = $log_path."/qtalk_search_".date("Y-m").".log";
    $loginfo = "[".date("Y-m-d H:i:s")."]:".$loginfo."\n";
    error_log($loginfo,3,$log_file);*/
}
/**
 * 发送邮件
 * @param type $title
